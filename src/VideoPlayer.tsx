...
const VideoPlayer = ({ url, title, onClose }: VideoPlayerProps) => {
  ...
  const triggerAnimation = (icon: string) => {
    setPulseIcon(icon);
    // Increased timeout slightly to 500ms for a smoother feel on seek
  ...
  // UPDATED: Now triggers animations for seeking
  ...
  return (
    <div 
      ...
      className={`relative w-full h-full bg-black overflow-hidden select-none transition-all duration-500
        ${isFullscreen ? 'fixed inset-0 z-[999]' : 'aspect-video'}
      `}
    ...
    >
      ...
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-20">
        <div 
          className={`w-20 h-20 bg-black/50 rounded-full flex items-center justify-center text-white transition-all duration-300 
            ${pulseIcon ? 'opacity-100 scale-125' : 'opacity-0 scale-100'}
          `}
        >
      ...
      <div 
        onClick={togglePlay}
        className={`absolute inset-0 cursor-pointer bg-gradient-to-t from-black/90 via-transparent to-black/40 transition-opacity duration-300 
          ${isHovering || isDragging || !isPlaying ? 'opacity-100' : 'opacity-0'}`}
      >
        {/* Top Bar */}
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-6">
              <div className="flex items-center gap-2">
                <button onClick={(e) => { e.stopPropagation(); skipTime(-5); }} className="text-white/60 hover:text-[#16C47F] transition-all"><BackIcon /></button>
                <button onClick={(e) => { e.stopPropagation(); togglePlay(); }} className="text-white hover:text-[#16C47F] w-8 text-center">
                  <span className="text-3xl">{isPlaying ? '⏸' : '▶'}</span>
                </button>
                <button onClick={(e) => { e.stopPropagation(); skipTime(5); }} className="text-white/60 hover:text-[#16C47F] transition-all"><ForwardIcon /></button>
              </div>
            ...
            <div className="flex items-center gap-4">
              <button onClick={(e) => { e.stopPropagation(); toggleFullscreen(); }} className="text-white/60 hover:text-[#16C47F] transition-all"><FullscreenIcon /></button>
              <div className="text-[10px] font-bold text-[#16C47F] uppercase tracking-tighter bg-[#16C47F]/10 px-2 py-1 rounded border border-[#16C47F]/20">Lecture Mode</div>
            </div>
          </div>
        </div>
      </div>
